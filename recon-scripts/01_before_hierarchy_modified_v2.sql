...
--------------------------------------------------------------------------------
-- UPDATED RECON: Account → Billinfo / MSISDN / Root Package counts (BEFORE)
--------------------------------------------------------------------------------
BEGIN
...
SELECT
  ...
  'Acct + Billinfo + MSISDN + Root Package counts (V2)' AS Criteria,
  a.account_no,
  a.poid_id0 AS account_poid,
  COUNT(DISTINCT b.poid_id0)  AS billinfo_count,
  COUNT(DISTINCT s.poid_id0)  AS msisdn_count,
  COUNT(DISTINCT pb.poid_id0) AS root_package_count
FROM   account_t a
JOIN   bal_grp_t bg
       ON a.poid_id0 = bg.account_obj_id0
JOIN   service_t s
       ON bg.poid_id0 = s.bal_grp_obj_id0
      AND bg.account_obj_id0 = s.account_obj_id0
JOIN   billinfo_t b
       ON bg.billinfo_obj_id0 = b.poid_id0
JOIN   service_alias_list_t sa
       ON s.poid_id0 = sa.obj_id0
      AND sa.rec_id = 1
JOIN   purchased_product_t p
       ON p.service_obj_id0 = s.poid_id0
      AND p.status = 1
JOIN   product_t pp
       ON pp.poid_id0 = p.product_obj_id0
LEFT JOIN purchased_bundle_t pb
       ON pb.poid_id0 = p.plan_obj_id0
      AND pb.account_obj_id0 = a.poid_id0
WHERE  s.poid_type = '/service/telco/gsm/telephony'
AND   ( NOT EXISTS (SELECT 1 FROM filter_accts)
