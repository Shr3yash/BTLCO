...
--------------------------------------------------------------------------------
-- RECON: Account → Billinfo / Telephony MSISDN / Root Package counts (AFTER)
--------------------------------------------------------------------------------
-- Drop output table if it exists
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE RECON_ACCT_BILL_SVC_PP_AFTER PURGE';
EXCEPTION
  WHEN OTHERS THEN
...
CREATE TABLE RECON_ACCT_BILL_SVC_PP_AFTER AS
WITH filter_accts AS (
  SELECT account_no FROM RECON_FILTER_ACCOUNTS
)
SELECT
  'After Hierarchy' AS Category,
