...
--------------------------------------------------------------------------------
-- Helper: Create filter table once (safe if already exists)
--------------------------------------------------------------------------------
BEGIN
  EXECUTE IMMEDIATE 'CREATE TABLE RECON_FILTER_ACCOUNTS (account_no VARCHAR2(64) PRIMARY KEY)';
EXCEPTION
  WHEN OTHERS THEN
...
-- Optional usage (run manually when you want to filter to specific accounts):
-- TRUNCATE TABLE RECON_FILTER_ACCOUNTS;
-- INSERT INTO RECON_FILTER_ACCOUNTS(account_no) VALUES (''1000561664'');
-- COMMIT;


--------------------------------------------------------------------------------
-- RECON: Account → Billinfo / Telephony MSISDN / Root Package counts (BEFORE)
--------------------------------------------------------------------------------
-- Drop output table if it exists
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE RECON_ACCT_BILL_SVC_PP_BEFORE PURGE';
EXCEPTION
  WHEN OTHERS THEN
...
CREATE TABLE RECON_ACCT_BILL_SVC_PP_BEFORE AS
WITH filter_accts AS (
  SELECT account_no FROM RECON_FILTER_ACCOUNTS
)
SELECT
  'Before Hierarchy' AS Category,
  'Acct + Billinfo + MSISDN + Root Package counts' AS Criteria,
  a.account_no,
  a.poid_id0                          AS account_poid,
  COUNT(DISTINCT b.poid_id0)          AS billinfo_count,
  COUNT(DISTINCT s.poid_id0)          AS msisdn_count,
  COUNT(DISTINCT p.poid_id0)          AS root_package_count
FROM
  account_t a
  LEFT JOIN billinfo_t b
    ON a.poid_id0 = b.account_obj_id0
  LEFT JOIN service_t s
    ON a.poid_id0 = s.account_obj_id0
   AND s.poid_type = '/service/telco/gsm/telephony'
  LEFT JOIN purchased_product_t p
    ON a.poid_id0 = p.account_obj_id0
WHERE
  -- If RECON_FILTER_ACCOUNTS is empty, run for ALL accounts.
  -- If populated, restrict to those account_nos.
  NOT EXISTS (SELECT 1 FROM filter_accts)
  OR a.account_no IN (SELECT account_no FROM filter_accts)
GROUP BY
  a.account_no, a.poid_id0
ORDER BY
  a.account_no, a.poid_id0;
