...
public class XMLGenerationUtils {
    ...
    // ---- NEW: epoch/ISO formatting helpers for Eff/CrtT ------------------------

    /**
     * Converts an epoch-like string to ISO-8601 instant with Z.
     * Supports:
     * - seconds since epoch (10 digits, e.g., "1583263532")
     * - milliseconds since epoch (13+ digits, e.g., "1583263532000")
     * If parsing fails (already ISO or non-numeric), returns the input as-is.
     */
    public static String formatEpochToIso(String epochLike) {
        ...
        try {
            ...
            return inst.toString(); // e.g., "2004-01-01T00:00:00Z"
        } catch (NumberFormatException e) {
            // Not a pure epoch; assume it's already an ISO-like string or leave as-is
            return v;
        }
    }

    /**
     * Writes <Eff> and <CrtT> inside the current <Act> block.
     * Reads columns ACC_EFFECTIVE_T and ACC_CREATED_T from the ResultSet,
     * converts epoch seconds/millis to ISO-8601 with Z, and writes tags only if non-empty.
     */
